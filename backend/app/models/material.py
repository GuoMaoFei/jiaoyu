...
class Material(Base):
    ...
    material_type = Column(Enum(MaterialType, name="material_type_enum"), default=MaterialType.OFFICIAL)
    pdf_asset_id = Column(String, ForeignKey("media_assets.id"), nullable=True)
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    
    nodes = relationship("KnowledgeNode", back_populates="material", cascade="all, delete-orphan")
...
class KnowledgeNode(Base):
    ...
    parent_id = Column(String, ForeignKey("knowledge_nodes.id"), nullable=True, index=True)
    ...
    pi_nodes_json = Column(JSON, nullable=True) # Full native PageIndex node objects array
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    ...
    children = relationship("KnowledgeNode", back_populates="parent", foreign_keys=[parent_id])
    questions = relationship("Question", back_populates="node")
    contents = relationship("KnowledgeContent", back_populates="node", cascade="all, delete-orphan")

class KnowledgeContent(Base):
    __tablename__ = "knowledge_contents"
    
    id = Column(String, primary_key=True, default=generate_uuid, index=True)
    knowledge_node_id = Column(String, ForeignKey("knowledge_nodes.id"), nullable=False, index=True)
...
class Question(Base):
    ...
    source_type = Column(Enum(QuestionSourceType, name="question_source_type_enum"), default=QuestionSourceType.OFFICIAL)
...
class MediaAsset(Base):
    ...
    access_scope = Column(Enum(AssetScope, name="asset_scope_enum"), default=AssetScope.PRIVATE)
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
