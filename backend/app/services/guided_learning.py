...
from app.models.lesson import LessonProgress, LessonStep
...
async def get_or_create_lesson(
    ...
    async with AsyncSessionLocal() as db:
        ...
        _pi_summary = ""
        if node and node.pi_nodes_json and isinstance(node.pi_nodes_json, list) and len(node.pi_nodes_json) > 0:
...
async def _fetch_or_generate_example(node_id: str) -> str:
    ...
    async with AsyncSessionLocal() as db:
        result = await db.execute(select(Question).where(Question.node_id == node_id).limit(1))
...
async def advance_lesson_step(
    ...
    async with AsyncSessionLocal() as db:
        ...
        if lesson.is_completed:
            ...
            step_prompt = base_prompt.format(node_title=node_title, content_preview=content_preview)
