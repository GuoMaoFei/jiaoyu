...
all_tools = [
    search_knowledge_tree, save_assessment,
    create_study_plan, get_material_node_list,
    get_node_questions, save_variant_question,
    get_chapter_health_report, get_mistake_summary,
]
...
async def supervisor_node(state: AgentState):
    ...
    print(f"--- MEMORY OVERLAY: avg_score={overlay['avg_health_score']}, "
    ...
    if node_id:
        async with AsyncSessionLocal() as db:
            ...
            if node:
                ...
                _node_summary = ""
                if node.pi_nodes_json and isinstance(node.pi_nodes_json, list) and len(node.pi_nodes_json) > 0:
                    _node_summary = node.pi_nodes_json[0].get("summary", "")
                tutor_ctx["node_content"] = _node_summary or f"本节主题：{node.title}。请使用 search_knowledge_tree 工具检索详细内容。"
