...
class SearchKnowledgeParams(BaseModel):
    ...
    current_node_id: Optional[str] = Field(None, description="The ID of the current knowledge node the student is studying.")
    expert_preference: str = Field("", description="Any historical mistakes or weak points to focus the search.")

@tool(args_schema=SearchKnowledgeParams)
async def search_knowledge_tree(query: str, material_id: str, student_id: str, current_node_id: Optional[str] = None, expert_preference: str = "") -> str:
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not current_node:
            ...
            stmt_fallback = select(KnowledgeNode).where(KnowledgeNode.material_id == material_id).order_by(KnowledgeNode.seq_num)
        ...
        stmt_pool = select(KnowledgeNode).where(
        ...
        structured_router = router_llm.with_structured_output(
            schema={"type": "object", "properties": {"selected_pi_ids": {"type": "array", "items": {"type": "string"}}}},
            method="json_mode"
        )
        
        try:
            from langchain_core.messages import SystemMessage
            route_res = structured_router.invoke([SystemMessage(content=router_prompt)])
        ...
        for c in contents:
            parent_title = next((n.title for n in historical_nodes if n.id == c.knowledge_node_id), "Unknown Chapter")
...
def create_search_knowledge_tree_tool(student_id: str, material_id: str, current_node_id: Optional[str] = None, expert_preference: str = ""):
    """
