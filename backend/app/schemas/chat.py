...
class ChatMessageRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(
        None, description="Existing session ID. If null, a new session is created."
    )
    node_id: Optional[str] = Field(
        None,
        description="The current active knowledge node ID, if learning a specific lesson",
    )
    lesson_step: Optional[str] = Field(
        None, description="The current guided learning step (IMPORT, EXPLAIN, etc.)"
    )
    message: str = Field(..., min_length=1, description="The student's message text")
    image_url: Optional[str] = Field(
        None, description="URL of an uploaded image (for photo-question feature)"
    )
    intent: Optional[str] = Field(
        None, description="Agent intent: 'chat', 'variant', 'reporter'"
    )
...
class AgentMessageResponse(BaseModel):
    ...
    tool_calls_made: Optional[List[str]] = Field(
        None, description="List of tool names called during this turn"
    )

    # Memory Overlay snapshot for debugging / frontend display
    health_score: Optional[int] = Field(
        None, description="Student's current avg health score"
    )
    weak_nodes_count: Optional[int] = Field(
        None, description="Number of weak nodes detected"
    )
