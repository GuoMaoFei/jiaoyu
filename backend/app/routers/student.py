...
async def create_student(
    request: StudentCreateRequest, db: AsyncSession = Depends(get_db)
):
...
async def get_student_profile(student_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if overlay["weak_nodes"]:
        weak_node_ids = overlay["weak_nodes"][:10]

        states_result = await db.execute(
            select(StudentNodeState).where(
                StudentNodeState.student_id == student_id,
                StudentNodeState.node_id.in_(weak_node_ids),
            )
        )
        ...
        titles = {row.id: row.title for row in nodes_result}

        for node_id in weak_node_ids:
            ns = states.get(node_id)
            if ns:
                weak_nodes.append(
                    NodeHealthResponse(
                        node_id=node_id,
...
async def activate_book(
    request: BookActivateRequest, db: AsyncSession = Depends(get_db)
):
    ...
    if existing.scalars().first():
        raise HTTPException(
            status_code=409, detail="Material already activated for this student"
        )
...
async def get_bookshelf(student_id: str, db: AsyncSession = Depends(get_db)):
    ...
    node_count_result = await db.execute(
        select(
            KnowledgeNode.material_id, func.count(KnowledgeNode.id).label("node_count")
        )
    ...
    for material in all_materials:
        ...
        books.append(
            BookshelfItemResponse(
                ...
                node_count=node_counts.get(material.id, 0),
...
@router.get(
    "/{student_id}/materials/{material_id}/nodes",
    response_model=StudentNodeListResponse,
)
async def get_student_nodes(
    student_id: str, material_id: str, db: AsyncSession = Depends(get_db)
):
    ...
    if not material_node_ids:
        return StudentNodeListResponse(
            student_id=student_id, material_id=material_id, node_states=[]
        )
    ...
    return StudentNodeListResponse(
        student_id=student_id, material_id=material_id, node_states=response_states
    )
...
async def get_mistakes(
    ...
    stmt = (
        select(StudentMistake, KnowledgeNode.title.label("node_title"))
        .join(KnowledgeNode, StudentMistake.node_id == KnowledgeNode.id)
    ...
    for mistake_obj, node_title in rows:
        mistakes.append(
            StudentMistakeResponse(
                ...
                next_review_date=mistake_obj.next_review_date.isoformat()
                if mistake_obj.next_review_date
                else None,
    ...
    return MistakeListResponse(
        student_id=student_id, mistakes=mistakes, total=len(mistakes)
    )
