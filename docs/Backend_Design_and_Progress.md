...
> **当前状态**：后端全部完成 ✅。五大 Agent + 10 个 LangGraph 工具 + 19 个 FastAPI 端点 + SSE 流式对话 + OCR 视觉 + 艾宾浩斯复习引擎。**V1.1 重构**：完成 KnowledgeNode/KnowledgeContent 骨肉分离 + 推理型 RAG 四步检索链 + 统一书架改造。**V1.2 优化**：实现方案 C 动态 LLM 选择（根据五步教学阶段分级调度模型）+ 学习舱自动讲解。**V1.3 学习计划重构**：多教材支持 + 按日期聚合展示 + 计划完成状态同步修复。**V1.4 OCR 缓存优化**：EasyOCR 集成 + 本地缓存机制，失败后可从缓存恢复。**V1.5 LLM 调用优化**：教材解析流水线引入"规则化优先+LLM回退"策略，LLM 调用减少 75-87%；详见 [教材解析流水线文档](Material_Parsing_Pipeline.md)。
